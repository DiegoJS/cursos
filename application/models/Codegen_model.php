...
class Codegen_model extends CI_Model {
    ...
    function getJoin($table, $fields, $where='',$join='',$rel=''){
        $this->db->select($fields);
        $this->db->from($table);
        if($where){
            $this->db->where($where);
  ...
  public function updateStock($producto, $medida,$cantidad)
  {
    $this->db->where('producto_id', $producto);
    $this->db->where('medida_id', $medida);
    $this->db->set('stock', 'stock-'.$cantidad, FALSE);
    $this->db->update('ecommerce_productos_medidas');
  }

  public function addStock($producto, $medida, $cantidad)
  {
    $this->db->where('producto_id', $producto);
    $this->db->where('medida_id', $medida);
    $this->db->set('stock', 'stock+'.$cantidad, FALSE);
